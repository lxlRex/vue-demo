  ...
  methods: {
    init() {
      this.$nextTick(() => {
        ...
        this.maxDistance =
          scrollContainer.offsetHeight - scrollBox.offsetHeight > 0
            ? scrollContainer.offsetHeight - scrollBox.offsetHeight
            : 0
      })
    },
    ...
    scroll(scrolling = false) {
      ...
      } else if (thisDistance >= -this.maxDistance) {
        // 上拉
        this.translateY = thisDistance
      } else {
        // 上拉超过最大距离
        this.translateY =
          scrolling && this.flexible
            ? -this.maxDistance + (thisDistance + this.maxDistance) / 10
            : -this.maxDistance
      }
    }
  },
